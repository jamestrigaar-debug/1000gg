...
(function testForeignTeamStrengthBaseline() {
  g.startCreation('easy');
  g.setSeed('foreign-team-baseline');
  const madrid = g.TEAM_DATABASE['Real Madrid'];
  const initial = madrid.attack + madrid.midfield + madrid.defence + madrid.manager;
  for (let i = 0; i < 12; i++) g.updateTeamStrengths();
  const after = madrid.attack + madrid.midfield + madrid.defence + madrid.manager;
  assert(after >= initial - 24, 'seasonal drift must not collapse Real Madrid toward the La Liga floor');
  assert(madrid.attack >= 90, 'seasonal drift must retain elite La Liga attack strength');
  console.log('Regression passed: foreign team strength baselines persist');
})();
