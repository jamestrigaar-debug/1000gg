...
(function () {
  ...
  const LEAGUE_WEIGHTS = {
    Elite: { goals: 1.0, minutes: 1.0, reputation: 1.0, wages: 1.0, compFactor: 0.28, europeWeight: 0.55 },
    Europe: { goals: 1.0, minutes: 0.95, reputation: 0.85, wages: 0.85, compFactor: 0.20, europeWeight: 0.45 },
    Mid: { goals: 1.02, minutes: 1.0, reputation: 0.70, wages: 0.70, compFactor: 0.09, europeWeight: 0.25 },
    Lower: { goals: 1.05, minutes: 1.0, reputation: 0.55, wages: 0.55, compFactor: 0.04, europeWeight: 0.10 },
    LaLiga: { goals: 1.0, minutes: 0.95, reputation: 0.95, wages: 0.85, compFactor: 0.26, europeWeight: 0.50 },
    SerieA: { goals: 0.95, minutes: 0.95, reputation: 0.85, wages: 0.75, compFactor: 0.24, europeWeight: 0.50 },
    Bundesliga: { goals: 1.05, minutes: 1.0, reputation: 0.88, wages: 0.75, compFactor: 0.24, europeWeight: 0.50 },
    MLS: { goals: 0.85, minutes: 1.05, reputation: 0.40, wages: 0.80, compFactor: 0.10, europeWeight: 0.05 },
    Saudi: { goals: 0.90, minutes: 1.05, reputation: 0.35, wages: 1.0, compFactor: 0.10, europeWeight: 0.05 },
    Championship: { goals: 0.80, minutes: 1.05, reputation: 0.35, wages: 0.40, compFactor: 0.08, europeWeight: 0.05 },
  };
  ...
  const FOREIGN_LEAGUE_KEYS = ["LaLiga", "SerieA", "Bundesliga", "Saudi", "MLS", "Championship"];
  const ALL_CLUBS = Object.keys(TEAM_DATABASE);
  const FOREIGN_LEAGUE_CLUBS = ALL_CLUBS.filter((c) => FOREIGN_LEAGUE_KEYS.includes(TEAM_DATABASE[c].league));
  ...
  function spin() {
    ...
    const iv = setInterval(() => {
      if (phase === "country") {
        ...
        target.innerHTML = `<div class="spinner-team">${o.flag} ${esc(c)}<span class="spinner-year">${o.story}</span></div>`;
      } else if (phase === "build") {
  ...
  function donorAttrGrid(key, pl) {
    const rows = [
      ...
      { k: "body", label: "PHY", v: Math.round((pl.fitness + pl.strength) / 2) },
    ...
    return `<div class="donor-attrs">${rows.map((r) =>
      `<div class="donor-attr${r.k === key ? " highlight" : ""}"><span>${r.label}</span><b>${r.v}</b></div>`
    ).join("")}</div>`;
  }
    ...
    const cards = squad.map(({ pl, idx }) => {
      ...
      return `
        <button class="donor-card${isUsed ? " used" : ""}" data-idx="${idx}"${isUsed ? " disabled" : ""}>
          ...
          ${hideStats ? "" : donorAttrGrid(key, pl)}
        </button>`;
    }).join("");
  ...
  function compilePlayer() {
    ...
    // Aerial mutation: small/quick build drafts a towering heading threat
    if (h <= 175 && headingDonor.heading >= 82) {
      attrs.heading = clamp(attrs.heading + 8, 40, 99);
      attrs.speed = clamp(attrs.speed - 5, 40, 99);
      attrs.fitness = clamp(attrs.fitness - 3, 40, 99);
      mutations.push("Aerial mutation — unnatural leap costs pace");
    }
    // Giant speedster mutation: tall frame drafts a pure pace merchant
    if (h >= 190 && speedDonor.speed >= 88) {
      attrs.speed = clamp(attrs.speed + 4, 40, 99);
      attrs.strength = clamp(attrs.strength + 3, 40, 99);
      attrs.fitness = clamp(attrs.fitness - 5, 40, 99);
      mutations.push("Giant speedster — the engine pays the price");
    }
  ...
  function renderConfirm() {
    ...
    card.innerHTML = `
      <div class="rating-hero">
        ...
        <div class="acad-chip">${state.player.origin.flag} ${state.country} · ${state.player.origin.story}</div>
      </div>
  ...
  function updateTeamStrengths() {
    ...
    for (const c of clubs) {
      ...
      t.attack = clamp(t.attack + drift, 55, 99);
      t.midfield = clamp(t.midfield + drift, 55, 99);
      t.defence = clamp(t.defence + drift, 55, 99);
      t.manager = clamp(t.manager + managerChange, 55, 99);
      ...
      if (change > 5) {
        ...
        t.attack = clamp(t.attack - pull, 55, 99);
        t.midfield = clamp(t.midfield - pull, 55, 99);
        t.defence = clamp(t.defence - pull, 55, 99);
        t.manager = clamp(t.manager - pull, 55, 99);
      } else if (change < -5) {
  ...
  function applyPlayerTransferImpact(club, leaving) {
    ...
    if (leaving) {
      TEAM_DATABASE[club].attack = clamp(TEAM_DATABASE[club].attack - impact, 55, 99);
    } else {
      TEAM_DATABASE[club].attack = clamp(TEAM_DATABASE[club].attack + impact, 55, 99);
    }
  }
  ...
  const SEASON_DECISIONS = [
    ...
    {
      id: "national_team", category: "INTERNATIONAL", weight: 4, req: { intlCaps: 1 },
    ...
    },
    {
      ...
      text: (n) => `The club signs a £${Math.floor(50 + Math.random() * 90)}m rival striker.`,
      choices: (ctx) => [
    ...
    },
    {
      ...
      text: (n) => `The fans are ${state.reputation >= 65 ? "singing your name" : "starting to turn"}.`,
      choices: (ctx) => [
    ...
    },
    {
      ...
      text: (n) => `New owners take over ${state.club}. They promise to either spend big or strip the squad.`,
      choices: (ctx) => [
        { label: "Back the new project", fx: { pillars: { Loyalty: 6, Adaptability: 4 } } },
        { label: "Demand assurances", fx: { rep: -2, pillars: { Ambition: 6, Ego: 4 } } },
        { label: "Ask to leave", fx: { rep: -3, forceTransfer: true, pillars: { Ambition: 8, Loyalty: -6 } } },
      ],
    },
    {
    ...
    },
    {
      ...
      text: (n) => `${state.club} faces a financial crisis. Wages may be cut.`,
      choices: (ctx) => [
        { label: "Take a wage cut", fx: { wealth: -6, rep: 5, pillars: { Loyalty: 10, Ego: -4 } } },
        { label: "Demand full payment", fx: { rep: -4, pillars: { Ego: 6, Loyalty: -6 } } },
        { label: "Leave for a stable club", fx: { rep: -2, forceTransfer: true, pillars: { Ambition: 6, Loyalty: -4 } } },
      ],
    },
    {
  ...
  const CAREER_MILESTONES = [
    {
      ...
      text: (n) => `At ${state.age}, ${n} must choose the path to the top.`,
      choices: (ctx) => [
    ...
    },
    {
      ...
      text: (n) => `At ${state.age}, the physical edge is fading. ${n} must reinvent the game.`,
      choices: (ctx) => [
    ...
    {
      id: "international_retirement", ageRange: [32, 35], once: true, req: { intlCaps: 10 }, weight: 6,
      text: (n) => `International retirement is on the table. ${n} can protect the club legs.`,
      choices: (ctx) => [
        { label: "Retire from internationals", fx: { attrChange: { key: "fitness", delta: 3 }, rep: -2, pillars: { Longevity: 10, Professionalism: 4 } } },
    ...
    },
    {
      ...
      text: (n) => `At ${state.age}, ${n} faces the final contract decision.`,
      choices: (ctx) => [
        ...
        { label: "Retire now", fx: { retireNow: true } },
      ],
    },
  ];
  ...
  function getEventWeight(ev, ctx) {
    ...
    // Determination drives career-shaping choices (agent, training, mentality)
    if (["AGENT", "TRAINING", "MENTALITY"].includes(ev.category) && state.determination != null) {
      w += (state.determination - 50) / 18;
    }
    // Injury risk and longevity push injury / late-career events
    if (ev.category === "INJURY") {
      w += (state.injuryRating || 50) / 28;
      w += (state.longevity || 50) / 45;
    }
    // Academy quality influences training and development opportunities
    ...
    // Playstyle nudges flavour events (e.g., target man / pace merchant stories)
    if (ev.playstyle && state.playstyle && ev.playstyle.includes(state.playstyle)) {
      w += 3;
    }
    return Math.max(0, w);
  }
  ...
  function applyEffectsRaw(fx, multiplier = 1) {
    ...
    if (fx.wealth) state.wealth = (state.wealth || 0) + Math.round((typeof fx.wealth === "function" ? fx.wealth() : fx.wealth) * multiplier);
    if (fx.fame) state.fame = (state.fame || 0) + Math.round((typeof fx.fame === "function" ? fx.fame() : fx.fame) * multiplier);
  ...
  function generateOffers(sd) {
    ...
    // Chance to include foreign elite clubs for ambitious players
    if (ambition >= 60 && state.reputation >= 50) {
      const foreignElite = FOREIGN_LEAGUE_CLUBS.filter((t) => t !== state.club && ["LaLiga", "SerieA", "Bundesliga"].includes(TEAM_DATABASE[t].league));
  ...
  // Forced offers to MLS/Saudi/Championship for aging players.
  function generateForcedDestinationOffers(sd) {
    ...
    if (age >= 30 && rep >= 35) {
      const pool = FOREIGN_LEAGUE_CLUBS.filter((t) => ["MLS", "Saudi"].includes(TEAM_DATABASE[t].league));
      const c = choice(pool);
      const offer = computeClubContractOffer(sd, c);
    ...
    if (age >= 27 && rep >= 25 && !offers.length) {
      const pool = FOREIGN_LEAGUE_CLUBS.filter((t) => TEAM_DATABASE[t].league === "Championship");
      const c = choice(pool);
      const offer = computeClubContractOffer(sd, c);
      if (!offer.refused) offers.push({ club: c, years: offer.years, playtime: offer.playtime, injuryRisk: offer.injuryRisk, wage: offer.wage, forced: true, foreign: true });
    }
  ...
  function renderCareerStats() {
    ...
    const recordsHtml = `
      <div class="records-table">
        ...
        <div class="rec-row"><span>PL Goals</span><span>${rec.plGoals.recordHolder}</span><span>${rec.plGoals.total}</span><span>${leagueG}</span></div>
        <div class="rec-row"><span>PL Assists</span><span>${rec.plAssists.recordHolder}</span><span>${rec.plAssists.total}</span><span>${a}</span></div>
        <div class="rec-row"><span>PL Appearances</span><span>${rec.plAppearances.recordHolder}</span><span>${rec.plAppearances.total}</span><span>${apps}</span></div>
        <div class="rec-row"><span>Career Goals</span><span>${rec.careerGoals.recordHolder}</span><span>${rec.careerGoals.total}</span><span>${g}</span></div>
        <div class="rec-row"><span>Career Assists</span><span>${rec.careerAssists.recordHolder}</span><span>${rec.careerAssists.total}</span><span>${a}</span></div>
        <div class="rec-row"><span>Intl Caps</span><span>${rec.intlCaps.recordHolder}</span><span>${rec.intlCaps.total}</span><span>${state.intlCaps}</span></div>
        <div class="rec-row"><span>Intl Goals</span><span>${rec.intlGoals.recordHolder}</span><span>${rec.intlGoals.total}</span><span>${intlG}</span></div>
        <div class="rec-row"><span>Ballon d'Or</span><span>${rec.ballonDors.recordHolder}</span><span>${rec.ballonDors.total}</span><span>${state.honours.ballonDors}</span></div>
        <div class="rec-row"><span>UCL Goals</span><span>${rec.championsLeagueGoals.recordHolder}</span><span>${rec.championsLeagueGoals.total}</span><span>${euroG}</span></div>
        <div class="rec-row"><span>World Cup Goals</span><span>${rec.worldCupGoals.recordHolder}</span><span>${rec.worldCupGoals.total}</span><span>—</span></div>
      </div>`;

    const plGoalRows = insertPlayerIntoTop10(TOP_10_PL_GOALS, leagueG, state.player.name || "You")
      .map((r, i) => `<div class="top10-row${r.mine ? " mine" : ""}"><span>${i + 1}</span><span>${r.name}</span><span>${r.total}</span></div>`)
      .join("");
    const plAssistRows = insertPlayerIntoTop10(TOP_10_PL_ASSISTS, a, state.player.name || "You")
      .map((r, i) => `<div class="top10-row${r.mine ? " mine" : ""}"><span>${i + 1}</span><span>${r.name}</span><span>${r.total}</span></div>`)
      .join("");
    const plAppRows = insertPlayerIntoTop10(TOP_10_PL_APPEARANCES, apps, state.player.name || "You")
      .map((r, i) => `<div class="top10-row${r.mine ? " mine" : ""}"><span>${i + 1}</span><span>${r.name}</span><span>${r.total}</span></div>`)
      .join("");
    const allTimeGoalRows = insertPlayerIntoTop10(TOP_10_ALL_TIME_GOALS, g, state.player.name || "You")
      .map((r, i) => `<div class="top10-row${r.mine ? " mine" : ""}"><span>${i + 1}</span><span>${r.name}</span><span>${r.total}</span></div>`)
      .join("");
    const intlCapRows = insertPlayerIntoTop10(TOP_10_INTL_CAPS, state.intlCaps, state.player.name || "You")
      .map((r, i) => `<div class="top10-row${r.mine ? " mine" : ""}"><span>${i + 1}</span><span>${r.name}</span><span>${r.total}</span></div>`)
      .join("");
    const intlGoalRows = insertPlayerIntoTop10(TOP_10_INTL_GOALS, intlG, state.player.name || "You")
      .map((r, i) => `<div class="top10-row${r.mine ? " mine" : ""}"><span>${i + 1}</span><span>${r.name}</span><span>${r.total}</span></div>`)
      .join("");
    ...
    el.innerHTML = `
      <div class="profile-grid">
        ...
        <div class="profile-card">
          ...
          <div class="pc-val">${(POSITIONS[state.position] || POSITIONS.ST).label}</div>
          <div class="pc-meta">${state.position}</div>
        </div>
        <div class="profile-card">
          <div class="pc-label">Origin</div>
          <div class="pc-val">${origin.flag} ${state.country}</div>
          <div class="pc-meta">${origin.story}</div>
        </div>
        <div class="profile-card">
  ...
  function proceedToTransfer(sd, intl) {
    ...
    if (clubSells) presentTransfer(generateOffers(sd), sd, intl, true);
    else if (wantsMove && !loyalStay) presentTransfer(generateOffers(sd), sd, intl, false);
    else if (state.age >= 27 && !["LaLiga", "SerieA", "Bundesliga", "Saudi", "MLS", "Championship"].includes(TEAM_DATABASE[state.club].league) && rand() < 0.25) {
  ...
  function presentTransfer(offers, sd, intl, forced) {
    const box = document.getElementById("season-action");
    ...
    const text = forced
      ? isForeignForced
        ? `A move abroad has come in for ${state.player.name}. Pick one of the offers — this is a one-time path that will define the rest of your career.`
  ...
  function computeClubContractOffer(sd, club) {
    ...
    if (age >= 34 && playtime === "Bench" && rand() < 0.35) refused = true; // clubs won't always sign a veteran benchwarmer

    // Agent influence and loyalty can save a borderline deal
    if (refused && rand() < agentInfluence) refused = false;
    if (refused && rand() < (loyalty - 50) / 200) refused = false;

    // Wage estimate (k/week) influenced by league wealth, reputation, performance, and age
    let wage = Math.round((rep * 5 + { Sensational: 25, Overperformed: 15, "Met Expectation": 5, Underperformed: 0, Flop: -5 }[perf] || 0) * leagueWeights.wages);
  ...
  function generateCareerSummary() {
    ...
    if (state.intlGoals >= 20) taglines.push(`a ${state.country} national hero`);
    else if (state.intlCaps >= 20) taglines.push(`a loyal ${state.country} servant`);
    ...
    const body = `${state.player.name} was ${taglines[0]}${taglines.length > 1 ? ", " + taglines.slice(1).join(" and ") : ""}.` +
      ` Over ${years} seasons as a ${pos.label} ${state.country} international, they scored <span class="highlight">${total} goals</span> in ${apps} appearances — ` +
    ...
    return { title: `${state.player.name} — ${playstyle}`, body };
  }
  ...
  window.__STRESS_TEST__ = {
    ...
    adjustPillar, getPillar, checkCareerMilestone, pickSeasonDecision, applyEffects,
