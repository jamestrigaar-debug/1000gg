...
(function () {
  ...
  function simulateSeason() {
    ...
    for (const h of seasonClubs) {
      for (const a of seasonClubs) {
        ...
        share *= fitMult * roleMult * styleMod * getTraitMatchMultiplier(clubData.tacticalStyle) * getPositionTacticalMultiplier(clubData.tacticalStyle) * (1 + (fm + momentum * 3) / 100) * (1 + clutchBonus * 0.12) * killerBonus;
    ...
    const compFactor = { Elite: 0.28, Europe: 0.20, Mid: 0.09, Lower: 0.04, LaLiga: 0.26, SerieA: 0.24, Bundesliga: 0.24 }[clubData.league] || 0.06;
    ...
    const leagueWeight = { Elite: 0.55, Europe: 0.45, Mid: 0.25, Lower: 0.10, LaLiga: 0.50, SerieA: 0.50, Bundesliga: 0.50 }[clubData.league] || 0.15;
    const europeGoals = Math.round(cupEuroGoals * leagueWeight);
  ...
  function presentSeasonDecision(sd, intl) {
    ...
    if (!ev) { presentEventQueue([], 0, sd, intl); return; }
  ...
  function presentTransfer(offers, sd, intl, forced) {
    ...
    box.querySelectorAll(".offer").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        moveToClub(o.club);
        if (isContractOffer) {
          signAndAdvance(o.years, sd, intl, `Signed a ${o.years}-year deal with ${state.club}.`, o.playtime);
        } else {
  ...
  function computeClubContractOffer(sd, club) {
    ...
    if (refused) return { years: 0, maxYears: 0, playtime, injuryRisk, refused: true };
    return { years: offerYears, maxYears, playtime, injuryRisk, refused: false };
  }
    ...
    box.innerHTML = `
      <div class="decision milestone-event">
        ...
        <div class="contract-offer">
          ...
          <div class="contract-terms">${offer.years}-year contract · ${roleEmoji} ${offer.playtime}</div>
  ...
  function signAndAdvance(years, sd, intl, message, role) {
    signContract(years);
  ...
  window.__STRESS_TEST__ = {
    ...
    recomputePlayerStats, simulateInternational, computeClubContractOffer, generateOffers, determineNaturalRole,
