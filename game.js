...
(function () {
  ...
  function donorAttrGrid(key, pl) {
    const rows = [
      ...
      { k: "mentality", label: "MEN", v: pl.mentalityRating },
    ];
  ...
  function compilePlayer() {
    ...
    // mentality: trait from the mentality donor; rating nudged by locker-room average
  ...
  function calculateInjuryProneness(a, academyTier, luck, injuryRating) {
    ...
    return Math.round(clamp((base + (injuryRating || 50)) / 2, 5, 95));
  }
  ...
  function calculateRetirementAge(injuryProneness, fitness, longevity) {
    ...
    return Math.round(clamp(34 + (100 - injuryProneness) / 22 + fitness / 45 + (long - 50) / 20, 32, 42));
  }
  ...
  function applySeasonalAttributeChanges(sd) {
    ...
    // Growth phase: young players improve — playstyle, mentality, origin, determination, potential and injury shape the path
    if (age <= 22) {
    ...
    // Decline phase: older players lose sharpness — longevity and injury rating soften or accelerate it
    if (age >= 33) {
      ...
      const decay = Math.max(0, 1 + Math.floor((age - 32) / 2) + longMod + injuryMod);
  ...
  function computeClubContractOffer(sd, club) {
    ...
    if (hasTrait("Iron Man")) injuryRisk = Math.max(5, injuryRisk - 20);
    if (hasTrait("Injury Prone")) injuryRisk = Math.min(95, injuryRisk + 15);
    ...
    // Agent influence can save a borderline deal
